 ...
 * 
 * SRPG_AuraSkill.js is required for this plugin to work. 
 * Please place this plugin below SRPG_AuraSkill.js in the plugin manager.
 * 
 ...
 * 
 * ============================================================================
 * Processes Executable via Event Command > Script
 * ============================================================================
 *  Note:
 ...
 * 
 * 動作にはSRPG_AuraSkill.jsが必要です。
 * このプラグインは管理画面でSRPG_AuraSkill.jsより下に配置してください。
 * 
...
(function () {
    ...
    const _SRPG_TerrainEffect_Game_ActionResult_pushRemovedState = Game_ActionResult.prototype.pushRemovedState;
    ...
    Window_SrpgTerrainEffect.prototype.contentsHeight = function() {
        return this.fittingHeight(4) - this.standardPadding() * 2;
    };
    ...
    Window_SrpgTerrainEffect.prototype.drawContents = function() {
        const lineHeight = this.lineHeight() - 10;
        const y = 2;
        let lineNum = 0;
        this.drawItemName(this._terrainEffect, 0, -2, this.contentsWidth());
        this.contents.fontSize -= 6;
        const text1 = this._terrainEffect.meta.terrainText1;
        if (text1) {
            lineNum += 1;
            this.drawText(text1, 0, y + lineHeight * lineNum, this.contentsWidth());
        }
        const text2 = this._terrainEffect.meta.terrainText2;
        if (text2) {
            lineNum += 1;
            this.drawText(text2, 0, y + lineHeight * lineNum, this.contentsWidth());
        }
        const text3 = this._terrainEffect.meta.terrainText3;
        if (text3) {
            lineNum += 1;
            this.drawText(text3, 0, y + lineHeight * lineNum, this.contentsWidth());
        }
        const text4 = this._terrainEffect.meta.terrainText4;
        if (text4) {
            lineNum += 1;
            this.drawText(text4, 0, y + lineHeight * lineNum, this.contentsWidth());
        }
        this.contents.fontSize += 6;
    };
    ...
    Scene_Map.prototype.createSrpgTerrainEffectWindow = function() {
        ...
        this._mapSrpgTerrainEffectWindow.x = (_windowPosition === 'left') ? 0 : Graphics.boxWidth - _windowWidth;
